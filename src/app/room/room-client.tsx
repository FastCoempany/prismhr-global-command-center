...
function Row({
  ...
  row: RoomRow;
  face: Face;
  foldOpen: string | null;
  setFold: (k: string | null) => void;
}) {
  ...
  // Filing and pasting must never land behind a closed fold: any capture
  // gesture reveals the container that will show its receipt.
  const revealDrop = () => {
    if (face === "marquee") setFold("today");
    if (face === "jamb") setFold("drop");
  };
  const revealOnFile = () => {
    if (face === "marquee") setFold("today");
    if (face === "jamb") setFold("sheet");
  };

  // ── The fold fragments (triptych 2026-08-13): one markup, three faces.
  // The Register's open wall composes ALL of them in the shipped order; the
  // Marquee's panes and the Jamb's doors open them one at a time.
  const fragNmline = () => (
    <>
        ...
        {row.outcome ? (
          ...
          <>
            ...
            <p className={styles.move}>
              Filed {row.loss!.date}: “{row.loss!.phrase}”. Your call: stamp the meter, or
              keep working it.
            </p>
            {row.canWrite && (
              <span className={styles.lossBtns}>
                  ...
                  {did ? (
                    ...
                    <>
                      ...
                      <form action={addFollowUp} className={styles.inline}>
                        <input type="hidden" name="label" value={t.body.slice(0, 140)} />
                        <input type="hidden" name="returnTo" value="/room" />
        ...
        >
          ...
          {mismatch && (
            <div className={styles.misfile}>
              <b>This reads like {mismatch.claim}</b>, not {mismatch.bound}. Nothing filed
              yet.
              <span className={styles.sdSuggActs}>
                <button
                  type="button"
  ...
  // The wall — the shipped two-column row, verbatim, from the fragments.
  const wall = (skipNmline: boolean) => (
    <div className={styles.row}>
      <div className={styles.deal}>
        {!skipNmline && fragNmline()}
        {fragDealRest()}
        {fragMove()}
        {fragOutstanding()}
      </div>
  ...
  // ── The counts: each numeral is the length of exactly the array its pane
  // maps — one derivation, no second bookkeeping.
  const nOpen = row.sheetOpen.filter((t) => !gone.has(t.id) && !doneIds.has(t.id)).length;
  const nDelayed = row.sheetDelayed.filter(
    (t) => !gone.has(t.id) && !backNow.has(t.id),
  ).length;
  const nDone =
    row.sheetDoneToday.filter((t) => !gone.has(t.id)).length +
    row.sheetOpen.filter((t) => !gone.has(t.id) && doneIds.has(t.id)).length;
  const nOwed = row.owed.filter((o) => !owedGone.has(o.key)).length;
  const nUnknown = row.gaps.filter((g) => !askGone.has(g.id)).length;
  const wallPassed = /wall passed/i.test(row.move);
  const courtTag =
    row.court.tone === "you"
      ? "YOU"
      : row.court.tone === "them" || row.court.tone === "quiet"
        ? "THEM"
        : "";

  // ── The Register: the all-or-nothing face. One gesture opens the entire
  // wall beneath the line; many walls may stand open; the counts cell is an
  // instrument reading, never a door.
  if (face === "register") {
    const open = foldOpen === "wall";
    return (
      <div className={styles.entry}>
        <div
          className={`${styles.line} ${open ? styles.lineOpen : ""}`}
          role="button"
          tabIndex={0}
          onClick={() => setFold(open ? null : "wall")}
          onKeyDown={(e) => {
            if (e.key === "Enter" || e.key === " ") {
              e.preventDefault();
              setFold(open ? null : "wall");
            }
          }}
        >
          <span className={`${styles.tick} ${styles[`tick_${row.health}`]}`} />
          <span className={styles.lineNm}>
            <Briefed on={row.briefed} />
            <Link
              href={`/accounts?focus=${row.accountId}`}
          ...
          <span className={styles.miniMeter} title={row.climb.label}>
            <span className={styles.miniTrack}>
              <span
                className={styles.miniGain}
          ...
          <span
            className={`${styles.lineMove} ${
              row.rank === 0 && !open && !row.outcome ? styles.lineLead : ""
            } ${row.thin ? styles.lineThin : ""}`}
            title={row.move}
          >
            {row.outcome ? (
              <span
              ...
              >
                {row.outcome.status === "won" ? "CLOSED WON" : "CLOSED LOST"} · Retire the
                row.
              </span>
            ) : (
              row.move
          ...
          <span
            ...
            title="✸ open · ⚑ owed · ⏲ delayed · ✓ done"
          >
            {courtTag && (
              <span className={styles.lineCourtTag} title={row.court.line}>
                {courtTag}
              </span>
            )}
            {nOpen > 0 && <span className={styles.cntAct}>✸{nOpen}</span>}
            {nOwed > 0 && <span className={styles.cntOwed}>⚑{nOwed}</span>}
            {nDelayed > 0 && <span className={styles.cntDly}>⏲{nDelayed}</span>}
            {nDone > 0 && <span className={styles.cntDone}>✓{nDone}</span>}
            {nOpen + nOwed + nDelayed + nDone === 0 && (
              <span className={styles.cntNil}>—</span>
            )}
          </span>
          <button
            type="button"
            className={styles.lineCaret}
            title={open ? "Fold it back." : "Open the wall."}
            onClick={(e) => {
              e.stopPropagation();
              setFold(open ? null : "wall");
            }}
  ...
  // ── The Marquee: the spotlight face. Headline over count handles; the
  // numerals ARE the doors; at most one pane stands open across the board.
  if (face === "marquee") {
    const pane = foldOpen;
    const openPane = (p: string) => setFold(pane === p ? null : p);
    return (
      <div
        className={styles.mqRow}
        onDragEnter={() => {
          if (pane !== "today") setFold("today");
        }}
      >
        {fragNmline()}
        <p
          className={`${styles.move} ${styles.mqMove} ${row.thin ? styles.thin : ""}`}
          title={row.court.line}
        >
          {row.move}
        </p>
        <div className={styles.mqStrip}>
          <button
            type="button"
            className={`${styles.mqHandle} ${wallPassed ? styles.mqRed : ""} ${
              pane === "today" ? styles.mqOn : ""
            }`}
            title={wallPassed ? "A wall passed. Open the sheet." : "Open the day sheet."}
            aria-expanded={pane === "today"}
            onClick={() => openPane("today")}
          >
            {nOpen + nDelayed} TODAY
          </button>
          {nDone > 0 && <span className={styles.mqDone}>{nDone} DONE</span>}
          <button
            type="button"
            className={`${styles.mqHandle} ${nOwed > 0 ? styles.mqInk : ""} ${
              pane === "owed" ? styles.mqOn : ""
            }`}
            title="Open what you owe."
            aria-expanded={pane === "owed"}
            onClick={() => openPane("owed")}
          >
            {nOwed} OWED
          </button>
          <button
            type="button"
            className={`${styles.mqHandle} ${pane === "unknown" ? styles.mqOn : ""}`}
            title="Open the asks."
            aria-expanded={pane === "unknown"}
            onClick={() => openPane("unknown")}
          >
            {nUnknown} UNKNOWN
          </button>
          <button
            type="button"
            className={`${styles.mqHandle} ${pane === "record" ? styles.mqOn : ""}`}
            title="Open the record."
            aria-expanded={pane === "record"}
            onClick={() => openPane("record")}
          >
            {row.recordTotal} EARLIER
          </button>
          <span className={styles.mqSpacer} />
          <span className={styles.mqCourt} title={row.court.line}>
            {row.court.line}
          </span>
        </div>
        {pane && (
          <div className={`${styles.rec} ${styles.mqPane}`}>
            {pane === "today" && (
              <>
                {fragMove()}
                {fragOutstanding()}
            ...
            {pane === "owed" && (
              <>
                <div className={styles.dayrule}>OWED</div>
                {fragOwed()}
              </>
            )}
            {pane === "unknown" && fragUnknown()}
            {pane === "record" && fragRecord()}
            <button
              type="button"
              className={styles.paneFold}
              onClick={() => setFold(null)}
            >
              Fold it ▴
            </button>
          </div>
  ...
  // ── The Jamb: the workbench face. Every door stays visible as a labeled
  // tab on the rail; many rows stand open; DROP keeps the record in reach.
  const door = foldOpen;
  const doors: { key: string; label: string; n: number | null; red?: boolean }[] = [
    { key: "sheet", label: "SHEET", n: nOpen + nDelayed, red: wallPassed },
    { key: "owed", label: "OWED", n: nOwed },
    { key: "unknown", label: "UNKNOWN", n: nUnknown },
    { key: "record", label: "RECORD", n: row.recordTotal },
    { key: "drop", label: "DROP", n: null },
  ];
  return (
    <div className={styles.jbRow}>
      <span className={`${styles.jbEdge} ${styles[`edge_${row.health}`]}`} />
      <div className={styles.jbBody}>
        {fragNmline()}
        <p className={`${styles.move} ${styles.jbMove} ${row.thin ? styles.thin : ""}`}>
          {row.move}
        </p>
        <div className={styles.jbSub}>
          {row.meta && <span className={styles.jbMeta}>{row.meta}</span>}
          <span className={styles.jbCourt}>{row.court.line}</span>
        </div>
        {door && (
          <div className={`${styles.rec} ${styles.jbPane}`}>
            {door === "sheet" && (
              <>
                {fragMove()}
                {fragOutstanding()}
            ...
            {door === "unknown" && fragUnknown()}
            {door === "record" && fragRecord()}
            {door === "drop" && (
              <>
                {fragDrop()}
                {fragRecord()}
      ...
      <div className={styles.jbRail}>
        {doors.map((d) => (
          <button
            key={d.key}
            type="button"
            className={`${styles.jbTab} ${door === d.key ? styles.jbTabOn : ""} ${
              d.red ? styles.jbTabRed : ""
            } ${d.n === 0 ? styles.jbTabNil : ""}`}
            title={d.red ? "A wall passed. Open the sheet." : undefined}
            aria-expanded={door === d.key}
            onClick={() => setFold(door === d.key ? null : d.key)}
          >
            <span>{d.label}</span>
            {d.n != null && <span className={styles.jbN}>{d.n}</span>}
          </button>
        ))}
      </div>
    </div>
...
export function RoomClient({
  ...
  // The triptych's face and the day's fold state — one store, three
  // cardinality models. A new Chicago day arrives fully folded.
  const [face, setFace] = useState<Face>("register");
  const [fold, setFoldState] = useState(() => ({
    day: "",
    register: {} as Record<string, boolean>,
    marquee: null as { accountId: string; pane: PaneKey } | null,
    jamb: {} as Record<string, DoorKey>,
  }));
  useEffect(() => {
    const t = setTimeout(() => {
      setFace(loadFace());
      setFoldState(loadFold());
    }, 0);
    return () => clearTimeout(t);
  }, []);
  useEffect(() => {
    if (fold.day) saveFold(fold);
  }, [fold]);
  const pickFace = (f: Face) => {
    setFace(f);
    saveFace(f);
  };
  const foldFor = (id: string): string | null =>
    face === "register"
      ? fold.register[id]
        ? "wall"
        : null
      : face === "marquee"
        ? fold.marquee?.accountId === id
          ? fold.marquee.pane
          : null
        : (fold.jamb[id] ?? null);
  const setFoldFor = (id: string, k: string | null) => {
    setFoldState((s) => {
      const day = s.day || chicagoDayOf();
      if (face === "register") {
        const register = { ...s.register };
        if (k) register[id] = true;
        else delete register[id];
        return { ...s, day, register };
      }
      if (face === "marquee")
        return { ...s, day, marquee: k ? { accountId: id, pane: k as PaneKey } : null };
      const jamb = { ...s.jamb };
      if (k) jamb[id] = k as DoorKey;
      else delete jamb[id];
      return { ...s, day, jamb };
    });
  };
  ...
  return (
    <div className={styles.wrap}>
      <div className={styles.bar}>
        ...
        <span className={styles.faceTabs}>
          {FACES.map((f) => (
            <button
              key={f}
              type="button"
              className={`${styles.faceTab} ${face === f ? styles.faceTabOn : ""}`}
              onClick={() => pickFace(f)}
            >
              {f.toUpperCase()}
            </button>
      ...
      {rows.map((r) => (
        <Row
          ...
          row={r}
          face={face}
          foldOpen={foldFor(r.cardId)}
          setFold={(k) => setFoldFor(r.cardId, k)}
        />
      ))}
