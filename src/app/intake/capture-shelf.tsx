...
function teamsBookmarklet(origin: string): string {
  const js =
    ...
    `window.open('${origin}/intranet?grab=1','_blank')` +
    `})()`;
  return `javascript:${js}`;
}
...
//
// Destination (pass 4's SalesNav ruling of 2026-09-25 — CLAUDE.md, The Chute :309): one door.
// The grab pastes into the Chute or a Drop like any capture and files as a
// note under its own SALESNAV ACCOUNTS head, which the queue's intent rules
// read (src/lib/groundwork/signals.ts). The tab the grab opens below is
// where the operator lands; the paste itself goes to the Chute or a Drop.
